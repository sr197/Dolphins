...
```
check ecotype for Anni 

```{r}
check <- readxl::read_xlsx("Ecotype_Location_Verification_20220715 (1).xlsx")
check$Longitude <- ifelse(check$Longitude > 1, check$Longitude*-1, check$Longitude)
...
check$State <- ifelse(check$Latitude < 34.1, "SC", check$State)
check$State <- ifelse(check$Latitude > 34.1 & check$Latitude < 37, "NC", check$State)
check$State <- ifelse(check$Latitude > 36.9 & check$Latitude < 38, "VA", check$State)
check$State <- ifelse(check$Latitude > 37.9 & check$Latitude < 39, "MD", check$State)
check$State <- ifelse(check$Latitude >38.9 & check$Latitude < 40, "NJ", check$State)
check$State <- ifelse(check$Latitude > 40 & check$Longitude < -71, "NY", check$State)
check$State <- ifelse(check$Latitude > 40 & check$Longitude < -69 & check$Longitude > -71, "MA", check$State)
check$State <- ifelse(check$Longitude > -68, "New England", check$State)

check$Ecotype <- ifelse(check$Longitude > -68, "Offshore", check$Ecotype)

states <- sf::st_as_sf(maps::map("state", plot = FALSE, fill = TRUE))
ggplot(data = states) + geom_sf() + geom_point(data=check, aes(x = Longitude, y = Latitude, colour = State)) + geom_text(data=check, aes(x = Longitude, y = Latitude, label = ID)) +
    coord_sf(xlim=c(-85, -65), ylim=c(25,43), expand = TRUE) 

ggplot(data = states) + geom_sf() + geom_point(data=check, aes(x = Longitude, y = Latitude, colour = ID)) +
    coord_sf(xlim=c(-85, -65), ylim=c(25,43), expand = TRUE) 

ggplot(data = states) + geom_sf() + geom_point(data =check, aes(x = Longitude, y = Latitude, colour = Ecotype)) +
    coord_sf(xlim=c(-85, -65), ylim=c(25,43), expand = TRUE)

ggplot(data = states) + geom_sf() + geom_point(data =check, aes(x = Longitude, y = Latitude, colour = State)) +
    coord_sf(xlim=c(-85, -65), ylim=c(25,43), expand = TRUE)

ggplot(data = states) + geom_sf() + geom_point(data =subset(check, check$ID %in% c(1,2)), aes(x = Longitude, y = Latitude, colour = State)) +
    coord_sf(xlim=c(-85, -65), ylim=c(25,43), expand = TRUE)

write.csv(check, "anni_coordinates.csv")

#add in bathymetrydata 
fgdb <- "~/Documents/ClimateOceanPlanning/Datasets/SABMASeafloorHabitats/Data/Seafloor.gdb" 
library(rgdal)
subset(ogrDrivers(), grepl("GDB", name))
fc_list <- ogrListLayers(fgdb)
print(fc_list)
fc <- readOGR(dsn=fgdb,layer="Bathymetry")
summary(fc)


fc <- sf::st_read("~/Documents/ClimateOceanPlanning/Datasets/SABMASeafloorHabitats/Data/Seafloor.gdb" , layer = "Bathymetry") 
```
...
cor <- corReorder(cor, order = "manual", vars=c(BAY.ANCHOVY, SPOTTED.HAKE, SMOOTH.DOGFISH, BLACK.SEA.BASS, LADY.CRAB, BULLNOSE.RAY, NORTHERN.SEAROBIN, BUTTERFISH, SPOT, BLUEFISH, STRIPED.SEAROBIN, STRIPED.ANCHOVY, WEAKFISH, NORTHERN.PUFFER, ATLANTIC.CROAKER, AbdPerArea, CLEARNOSE.SKATE, SUMMER.FLOUNDER, SMALLMOUTH.FLOUNDER, WINDOWPANE, ATLANTIC.SILVERSIDE, BLUEBACK.HERRING, SPINY.DOGFISH, WINTER.SKATE, ATLANTIC.ROCK.CRAB, ATLANTIC.MACKEREL, SILVER.HAKE))

cor <-  as.data.frame(cor) 
rownames(cor) <- colnames(cor)


s_mat <- corReorder(s_mat, order = "manual", vars=c(BAY.ANCHOVY, SPOTTED.HAKE, SMOOTH.DOGFISH, BLACK.SEA.BASS, LADY.CRAB, BULLNOSE.RAY, NORTHERN.SEAROBIN, BUTTERFISH, SPOT, BLUEFISH, STRIPED.SEAROBIN, STRIPED.ANCHOVY, WEAKFISH, NORTHERN.PUFFER, ATLANTIC.CROAKER, AbdPerArea, CLEARNOSE.SKATE, SUMMER.FLOUNDER, SMALLMOUTH.FLOUNDER, WINDOWPANE, ATLANTIC.SILVERSIDE, BLUEBACK.HERRING, SPINY.DOGFISH, WINTER.SKATE, ATLANTIC.ROCK.CRAB, ATLANTIC.MACKEREL, SILVER.HAKE))
...
ycor <- corReorder(ycor, order = "manual", vars=c(BAY.ANCHOVY, SPOTTED.HAKE, SMOOTH.DOGFISH, BLACK.SEA.BASS, LADY.CRAB, BULLNOSE.RAY, NORTHERN.SEAROBIN, BUTTERFISH, SPOT, BLUEFISH, STRIPED.SEAROBIN, STRIPED.ANCHOVY, WEAKFISH, NORTHERN.PUFFER, ATLANTIC.CROAKER, AbdPerArea, CLEARNOSE.SKATE, SUMMER.FLOUNDER, SMALLMOUTH.FLOUNDER, WINDOWPANE, ATLANTIC.SILVERSIDE, BLUEBACK.HERRING, SPINY.DOGFISH, WINTER.SKATE, ATLANTIC.ROCK.CRAB, ATLANTIC.MACKEREL, SILVER.HAKE))
...
cor2 <- out[["parameters"]][["ematrix"]]
cor2 <- corReorder(cor2, order = "manual", vars=c(BAY.ANCHOVY, SPOTTED.HAKE, SMOOTH.DOGFISH, BLACK.SEA.BASS, LADY.CRAB, BULLNOSE.RAY, NORTHERN.SEAROBIN, BUTTERFISH, SPOT, BLUEFISH, STRIPED.SEAROBIN, STRIPED.ANCHOVY, WEAKFISH, NORTHERN.PUFFER, ATLANTIC.CROAKER, AbdPerArea, CLEARNOSE.SKATE, SUMMER.FLOUNDER, SMALLMOUTH.FLOUNDER, WINDOWPANE, ATLANTIC.SILVERSIDE, BLUEBACK.HERRING, SPINY.DOGFISH, WINTER.SKATE, ATLANTIC.ROCK.CRAB, ATLANTIC.MACKEREL, SILVER.HAKE))
...
betas_tidy$species <- factor(betas_tidy$species, levels=c("BAY.ANCHOVY", "SPOTTED.HAKE", "SMOOTH.DOGFISH", "BLACK.SEA.BASS", "LADY.CRAB", "BULLNOSE.RAY", "NORTHERN.SEAROBIN", "BUTTERFISH", "SPOT", "BLUEFISH", "STRIPED.SEAROBIN", "STRIPED.ANCHOVY", "WEAKFISH", "NORTHERN.PUFFER", "ATLANTIC.CROAKER", "AbdPerArea", "CLEARNOSE.SKATE", "SUMMER.FLOUNDER", "SMALLMOUTH.FLOUNDER", "WINDOWPANE", "ATLANTIC.SILVERSIDE", "BLUEBACK.HERRING", "SPINY.DOGFISH", "WINTER.SKATE", "ATLANTIC.ROCK.CRAB", "ATLANTIC.MACKEREL", "SILVER.HAKE"))
...
p
ggsave(filename= "Figures/supp_gjam_coastal.jpg", plot=p, width = 50, height=50, units=c("cm"), dpi=500)
...
colnames(coastal_table) <- c("Names", "p-value (glm)", "coefficient (glm)", "deviance (glm)", "aic (glm)", "importance (cforest)", "spearman (data)", "Env covariance", "residual correlation", "species order")
...
mod_base <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth  + factor(month) , data = coastal, control = cforest_unbiased(ntree= 500))

mod_king <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth + factor(month) + RED.DRUM + SPOT + ATLANTIC.CROAKER + WEAKFISH + SILVER.PERCH + BANDED.DRUM + STRIPED.ANCHOVY + SPINY.DOGFISH , data = coastal, control = cforest_unbiased(ntree= 500))
...
mod_king <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth + factor(month) + RED.DRUM + SPOT + ATLANTIC.CROAKER + WEAKFISH + SILVER.PERCH + BANDED.DRUM + STRIPED.ANCHOVY + SPINY.DOGFISH , data = train, control = cforest_unbiased(ntree= 500))
...
mod_base <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth + factor(month), data = coastal_pa, control = cforest_unbiased(ntree= 500))

mod_king <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth  + factor(month) + RED.DRUM + SPOT + ATLANTIC.CROAKER + WEAKFISH + SILVER.PERCH + BANDED.DRUM + STRIPED.ANCHOVY + SPINY.DOGFISH, data = coastal_pa, control = cforest_unbiased(ntree= 500))
...
ydata_coastal_pa <- ydata_coastal_pa[,colSums(ydata_coastal_pa != 0) > 17] # I want non-zero rows 
 ...
 ml <- list(ng = 20000, burnin = 8000, typeNames = 'PA')
...
# BLUEFISH, BUTTERFISH, SMOOTH.DOGFISH, BLACK.SEA.BASS, SPOTTED.HAKE, SCLEARNOSE.SKATE, NORTHERN.SEAROBIN, ATLANTIC.ROCK.CRAB, SUMMER.FLOUNDER, AbdPerArea, ROSETTE.SKATE, COMMON.OCTOPUS, SILVER.HAKE, GOOSEFISH, SPINY.DOGFISH, SEVENSPINE.BAY.SHRIMP, ATLANTIC.MACKEREL, BLUEBACK.HERRING, AMERICAN.SHAD, WINTER.SKATE, WINDOWPANE

cor <- corReorder(cor, order = "manual", vars=c(BLUEFISH, BUTTERFISH, SMOOTH.DOGFISH, BLACK.SEA.BASS, SPOTTED.HAKE, CLEARNOSE.SKATE, NORTHERN.SEAROBIN, ATLANTIC.ROCK.CRAB, SUMMER.FLOUNDER, AbdPerArea, ROSETTE.SKATE, COMMON.OCTOPUS, SILVER.HAKE, GOOSEFISH, SPINY.DOGFISH, SEVENSPINE.BAY.SHRIMP, ATLANTIC.MACKEREL, BLUEBACK.HERRING, AMERICAN.SHAD, WINTER.SKATE, WINDOWPANE))

cor <-  as.data.frame(cor) 
rownames(cor) <- colnames(cor)


s_mat <- corReorder(s_mat, order = "manual", vars=c(BLUEFISH, BUTTERFISH, SMOOTH.DOGFISH, BLACK.SEA.BASS, SPOTTED.HAKE, CLEARNOSE.SKATE, NORTHERN.SEAROBIN, ATLANTIC.ROCK.CRAB, SUMMER.FLOUNDER, AbdPerArea, ROSETTE.SKATE, COMMON.OCTOPUS, SILVER.HAKE, GOOSEFISH, SPINY.DOGFISH, SEVENSPINE.BAY.SHRIMP, ATLANTIC.MACKEREL, BLUEBACK.HERRING, AMERICAN.SHAD, WINTER.SKATE, WINDOWPANE))
...
ycor <- cor(ycor, method = "spearman")
ycor <- corReorder(ycor, order = "manual", vars=c(BLUEFISH, BUTTERFISH, SMOOTH.DOGFISH, BLACK.SEA.BASS, SPOTTED.HAKE, CLEARNOSE.SKATE, NORTHERN.SEAROBIN, ATLANTIC.ROCK.CRAB, SUMMER.FLOUNDER, AbdPerArea, ROSETTE.SKATE, COMMON.OCTOPUS, SILVER.HAKE, GOOSEFISH, SPINY.DOGFISH, SEVENSPINE.BAY.SHRIMP, ATLANTIC.MACKEREL, BLUEBACK.HERRING, AMERICAN.SHAD, WINTER.SKATE, WINDOWPANE))
 # ycor <- ifelse(ycor > .01, .01, ycor)
 # ycor <- ifelse(ycor < -.01, -.01, ycor)
...
cor2 <- out[["parameters"]][["ematrix"]]
cor2 <- corReorder(cor2, order = "manual", vars=c(BLUEFISH, BUTTERFISH, SMOOTH.DOGFISH, BLACK.SEA.BASS, SPOTTED.HAKE, CLEARNOSE.SKATE, NORTHERN.SEAROBIN, ATLANTIC.ROCK.CRAB, SUMMER.FLOUNDER, AbdPerArea, ROSETTE.SKATE, COMMON.OCTOPUS, SILVER.HAKE, GOOSEFISH, SPINY.DOGFISH, SEVENSPINE.BAY.SHRIMP, ATLANTIC.MACKEREL, BLUEBACK.HERRING, AMERICAN.SHAD, WINTER.SKATE, WINDOWPANE))
# cor2 <- ifelse(cor2 > 1, 1, cor2)
# cor2 <- ifelse(cor2 < -1, -1, cor2)
cor2 <-  as.data.frame(cor2) 
rownames(cor2) <- colnames(cor2)
...
betas_tidy$species <- factor(betas_tidy$species, levels=c("BLUEFISH", "BUTTERFISH", "SMOOTH.DOGFISH", "BLACK.SEA.BASS", "SPOTTED.HAKE", "CLEARNOSE.SKATE", "NORTHERN.SEAROBIN", "ATLANTIC.ROCK.CRAB", "SUMMER.FLOUNDER", "AbdPerArea", "ROSETTE.SKATE", "COMMON.OCTOPUS", "SILVER.HAKE", "GOOSEFISH", "SPINY.DOGFISH", "SEVENSPINE.BAY.SHRIMP", "ATLANTIC.MACKEREL", "BLUEBACK.HERRING", "AMERICAN.SHAD", "WINTER.SKATE", "WINDOWPANE"))
...
colnames(offshore_table) <- c("Names", "p-value (glm)", "coefficient (glm)", "deviance (glm)", "aic (glm)", "importance (cforest)", "spearman (data)", "Env covariance", "residual correlation", "species order")
...
xdata <- offshore %>% dplyr::select(SST, SSAL, BSAL, BT,depth,  month, SPOTTED.HAKE, ROSETTE.SKATE, CLEARNOSE.SKATE, SMOOTH.DOGFISH, ROUND.HERRING, SOUTHERN.KINGFISH, NORTHERN.SEAROBIN, SILVER.HAKE, WEAKFISH)
...
mod_king <- glm(AbdPerArea ~ SST + SSAL + BSAL + BT + depth + factor(month) + SPOTTED.HAKE + ROSETTE.SKATE + SMOOTH.DOGFISH + CLEARNOSE.SKATE + ROUND.HERRING + SOUTHERN.KINGFISH + NORTHERN.SEAROBIN + WEAKFISH, data = train)

xdata <- test %>% dplyr::select(SST, SSAL, BSAL, BT,depth,  month, SPOTTED.HAKE, ROSETTE.SKATE, SMOOTH.DOGFISH, CLEARNOSE.SKATE, ROUND.HERRING, SOUTHERN.KINGFISH, NORTHERN.SEAROBIN, SILVER.HAKE, WEAKFISH)
...
mod_king <- glm(AbdPerArea ~ SST + SSAL + BSAL + BT + depth  + factor(month) + SPOTTED.HAKE + ROSETTE.SKATE + SMOOTH.DOGFISH + CLEARNOSE.SKATE + ROUND.HERRING + SOUTHERN.KINGFISH + NORTHERN.SEAROBIN+ SILVER.HAKE+ WEAKFISH, data = offshore_pa, family = binomial(link = "logit"))

xdata <- offshore_pa %>% dplyr::select(SST, SSAL, BSAL, BT,depth, month, SST, SSAL, BSAL, BT,depth,  month, SPOTTED.HAKE, ROSETTE.SKATE, SMOOTH.DOGFISH, CLEARNOSE.SKATE, ROUND.HERRING, SOUTHERN.KINGFISH, NORTHERN.SEAROBIN, SILVER.HAKE, WEAKFISH)
...
mod_king <- glm(AbdPerArea ~ SST + SSAL + BSAL + BT + depth  + factor(month) + SPOTTED.HAKE + ROSETTE.SKATE + SMOOTH.DOGFISH + CLEARNOSE.SKATE + ROUND.HERRING + SOUTHERN.KINGFISH + NORTHERN.SEAROBIN + SILVER.HAKE+ WEAKFISH, data = train, family = binomial(link = "logit"))

xdata <- test %>% dplyr::select(SST, SSAL, BSAL, BT,depth, month, SST, SSAL, BSAL, BT,depth,  month, SPOTTED.HAKE, ROSETTE.SKATE, SMOOTH.DOGFISH, CLEARNOSE.SKATE, ROUND.HERRING, SOUTHERN.KINGFISH, NORTHERN.SEAROBIN, SILVER.HAKE, WEAKFISH)
...
mod_base <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth  + factor(month) , data = offshore, control = cforest_unbiased(ntree= 500))

mod_king <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth + factor(month) + SPOTTED.HAKE + ROSETTE.SKATE + SMOOTH.DOGFISH + CLEARNOSE.SKATE + ROUND.HERRING + SOUTHERN.KINGFISH + WEAKFISH + NORTHERN.SEAROBIN + SILVER.HAKE , data = offshore, control = cforest_unbiased(ntree= 500))
...
mod_king <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth + factor(month) + SPOTTED.HAKE + ROSETTE.SKATE + SMOOTH.DOGFISH + CLEARNOSE.SKATE + ROUND.HERRING + SOUTHERN.KINGFISH + WEAKFISH + NORTHERN.SEAROBIN + SILVER.HAKE, data = train, control = cforest_unbiased(ntree= 500))

xdata <- test %>% dplyr::select(SST, SSAL, BSAL, BT,depth,  month, SPOTTED.HAKE, ROSETTE.SKATE, CLEARNOSE.SKATE, SMOOTH.DOGFISH, ROUND.HERRING, SOUTHERN.KINGFISH, NORTHERN.SEAROBIN, SILVER.HAKE, WEAKFISH)
...
mod_base <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth + factor(month), data = offshore_pa, control = cforest_unbiased(ntree= 500))

mod_king <- cforest(AbdPerArea ~ SST + SSAL + BSAL + BT + depth  + factor(month) + SPOTTED.HAKE + ROSETTE.SKATE + SMOOTH.DOGFISH + CLEARNOSE.SKATE + ROUND.HERRING + SOUTHERN.KINGFISH + WEAKFISH + NORTHERN.SEAROBIN + SILVER.HAKE, data = offshore_pa, control = cforest_unbiased(ntree= 500))

xdata <- offshore_pa %>% dplyr::select(SST, SSAL, BSAL, BT,depth, month, SST, SSAL, BSAL, BT,depth,  month, SPOTTED.HAKE, ROSETTE.SKATE, CLEARNOSE.SKATE, SMOOTH.DOGFISH, ROUND.HERRING, SOUTHERN.KINGFISH, NORTHERN.SEAROBIN, SILVER.HAKE, WEAKFISH)
